...
describe('htmlToSlate top-level wrapper elements', () => {
  ...
  it('lifts blocks when an unmapped element beside them produces no content', () => {
    ...
    ).toEqual([{ type: 'p', children: [{ text: 'a' }] }]);
  });
  ...
  it('lifts when liftWrappedBlocks is not set on a custom config', () => {
    ...
    expect(htmlToSlate('<div><p>a</p></div>', config)).toEqual([
      { type: 'p', children: [{ text: 'a' }] },
    ]);
  });
...
describe('htmlToSlate with liftWrappedBlocks: false', () => {
  ...
  const cases: [string, string, unknown[]][] = [
    ...
    [
      ...
      '<article><section><h1>t</h1><p>a</p></section></article><p>b</p>',
      [
        { children: [{ type: 'h1', children: [{ text: 't' }] }, p('a')] },
        p('b'),
      ],
    ],
    [
  ...
  describe.each([
    ...
    it.each(cases)('keeps blocks nested inside %s', (_case, html, expected) => {
  ...
  it('still ignores <head> content in a full HTML document', () => {
    const html =
      '<html><head><title>Doc title</title></head><body><p>a</p></body></html>';
    expect(
      htmlToSlate(html, { ...htmlToSlateConfig, liftWrappedBlocks: false }),
    ).toEqual([{ children: [p('a')] }]);
  });
